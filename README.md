...
it running end to end before adapting it.

**Long-term goal:** get end-to-end QA reliable enough that a GenAI coding
agent's pull request can merge on its own say-so, without a human review
gating it first. That only works if every stage an agent's change passes
through -- local dev, secrets/credentials provisioning, and CI test runs
against a real environment -- behaves the same way every time, for an agent
as much as for a human. Seal is that stage: a thin, consistent wrapper
around Tilt that makes those steps reproducible enough to trust as a merge
gate, not just a local convenience.
